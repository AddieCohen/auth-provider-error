...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('vscode-authenticationprovider-sample.login', async () => {
		...
		let session;
		try {
			session = await vscode.authentication.getSession(AzureDevOpsAuthenticationProvider.id, [], {createIfNone: true});
		} catch (error) {
			if (error instanceof LoginTimeOutError) {
				console.log(`error LoginTimeOutError, name is: ${error.name}`);
			} else {
				console.log(`error is not LoginTimeOutError, name is: ${(error as Error).name}`);
			}
		}
